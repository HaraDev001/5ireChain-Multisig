...
impl<T: Config> Pallet<T> {
	/// transfor_vote_weights transform the vote weights into weights as decsribed
	pub fn transform_vote_weights(stake: u64, total_stake: u128, esg_score: u64) -> u64 {
		let stake_percentage = (100000 * (stake as u128)) / total_stake;
		// multiply stake percentage the multiply it by a 1000 to have
		// larger weight for handling decimals as u32
		// change the esg score aswell to be max 100,000
		let score = esg_score * 1000;
		let final_weight = stake_percentage as u64 + score;
		final_weight
	}

	/// The reverse of transform_vote_weights
	pub fn detransform_vote_weights(stake: u64, total_stake: u128, esg_score: u64) -> u64 {
		// remove the esg part of the stake. Reverse of stake_percentage = (100000 * (stake as u128)) / total_stake;
		let stake_part = stake as u128 - (esg_score as u128 * 1000);
		// reverse from the percentage back to the original stake
		let res = (stake_part * total_stake) / 100000;
		res as u64
	}

	// calculate total stake from validator
	pub fn total_stake() -> u128 {
		let total_validators = Validators::<T>::iter_keys()
			.map(|validator| {
				let validator_stake: BalanceOf<T> = Self::bonded(validator)
					.and_then(Self::ledger)
					.map(|l| l.active)
					.unwrap_or_default();
				// Convert primitive types in Rust into Substrate types like Balance, ... and vice versa
				validator_stake.saturated_into::<u128>()
			})
			.collect::<Vec<u128>>();

		let total_nominators = Nominators::<T>::iter_keys()
			.map(|nominator| {
				let nominator_stake: BalanceOf<T> = Self::bonded(nominator)
					.and_then(Self::ledger)
					.map(|l| l.active)
					.unwrap_or_default();
				// Convert primitive types in Rust into Substrate types like Balance, ... and vice versa
				nominator_stake.saturated_into::<u128>()
			})
			.collect::<Vec<u128>>();

		let total = total_validators
			.iter()
			.sum::<u128>()
			.saturating_add(total_nominators.iter().sum::<u128>());

		total
	}

	// calculate esg-score for nominator based on esg-score validator that be nominated by nominators
	pub fn esg_score_nominator(nominator: &T::AccountId) -> Result<u32, Error<T>> {
		// get validator that nominator elect

		// let nominators = Nominators::<T>::get(&nominator).expect("No nominator exists");
		// let nominated_validators = nominators.targets;

		let nominated_validators = Nominators::<T>::get(&nominator)
			.unwrap_or(Nominations { targets: vec![], submitted_in: 0, suppressed: false })
			.targets;

		// check nominated validators exist
		// ensure!(!nominated_validators.is_empty(), Error::<T>::EmptyTargets);
		if nominated_validators.is_empty() {
			return Ok(0u32)
		}
		let mut esg_nominator = 0u32;

		// add all of esg-score validators that nominators elect
		for val in nominated_validators.into_iter() {
			esg_nominator += T::EsgData::get_esg_score(&val).unwrap_or(0u8) as u32;
		}
	...
	pub(crate) fn try_trigger_new_era(
		...
		let exposures = &mut Self::collect_exposures(election_result);
		let cur_era = Self::current_era().unwrap_or(0);
		let total_stake = Self::total_stake();

		// change the exposures back to detransformed values
		for (stash, exposure) in exposures.iter_mut() {
			let esg_score = Self::eras_esg_score(&cur_era, stash.clone());
			let mut others_vec = vec![];
			for id_exp in &exposure.others {
				let mut exp = id_exp.clone();
				exp.value = Self::detransform_vote_weights(
					id_exp.value.saturated_into::<u64>(),
					total_stake,
					esg_score,
				)
			...
			*exposure = Exposure {
				total: Self::detransform_vote_weights(
					exposure.total.saturated_into::<u64>(),
					total_stake,
					esg_score,
				)
				.saturated_into::<BalanceOf<T>>(),
				own: Self::detransform_vote_weights(
					exposure.own.saturated_into::<u64>(),
					total_stake,
					esg_score,
				)
				.saturated_into::<BalanceOf<T>>(),
		...
		Some(Self::trigger_new_era(start_session_index, exposures.to_vec()))
	}
...
impl<T: Config> ElectionDataProvider<T::AccountId, BlockNumberFor<T>> for Pallet<T> {
	...
	fn voters(
		...
		let voters = &mut Self::get_npos_voters(maybe_max_len);
		debug_assert!(maybe_max_len.map_or(true, |max| voters.len() <= max));

		// Transform the voter's weight before supplying them to the validator
		let total_stake = Self::total_stake();
		for (voter, weight, validators) in voters.iter_mut() {
			let score = T::EsgData::get_esg_score(&validators[0]).unwrap_or(0);
			let cur_era = Self::current_era().unwrap_or(0);
			<ErasEsgScore<T>>::insert(cur_era, voter, score as u64);
			*weight = Self::transform_vote_weights(*weight, total_stake.into(), score as u64)
				as VoteWeight;
		}
