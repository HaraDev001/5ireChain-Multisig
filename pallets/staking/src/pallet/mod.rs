...
pub mod pallet {
	...
	#[pallet::config]
	pub trait Config:
		frame_system::Config + SendTransactionTypes<Call<Self>> + esg_fetcher::Config
	{
		/// The ESG fetcher
		type EsgData: Score<Self>;
	...
	/// The Esg Score of a validator in that era. This is the esg score that has been used to
	/// transform the weight for the vote in era i and should be used to transform the voteweight in
	/// era i+1
	#[pallet::storage]
	#[pallet::getter(fn eras_esg_score)]
	pub type ErasEsgScore<T: Config> =
		StorageDoubleMap<_, Twox64Concat, EraIndex, Twox64Concat, T::AccountId, u64, ValueQuery>;
