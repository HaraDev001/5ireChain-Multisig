...
use super::{Event, *};
use frame_election_provider_support::{ElectionProvider, SortedListProvider, Support, VoteWeight};
use frame_support::{
...
#[test]
fn transform_vote_weights_works() {
	ExtBuilder::default().build_and_execute(|| {
		let weights = <Pallet<Test>>::transform_vote_weights(100, 1000, 50);
		// we have a 10% stake and an esg score of 50, so we should have a total weight of 10 * 1000 + 50 * 1000 = 60000
		assert_eq!(weights, 60000)
	});
}

#[test]
fn detransform_vote_weights_works() {
	let weights = <Pallet<Test>>::transform_vote_weights(100, 1000, 50);
	// we have a 10% stake and an esg score of 50, so we should have a total weight of 10 * 1000 + 50 * 1000 = 60000
	assert_eq!(weights, 60000);
	// Here we use the previous total stake
	let detransformed = <Pallet<Test>>::detransform_vote_weights(60000, 1000, 50);
	assert_eq!(detransformed, 100)
}
...
mod election_data_provider {
	use super::*;
	use frame_election_provider_support::{onchain::Error::DataProvider, ElectionDataProvider};
	...
	#[test]
	fn voters_include_self_vote_weight() {
		ExtBuilder::default().nominate(false).build_and_execute(|| {
			let voters = Staking::voters(None).unwrap();
			for (_, v, _) in voters.iter() {
				println!("{}", *v)
			}
			let esg_weight: Vec<u64> = <Validators<Test>>::iter()
				.map(|(x, _)| {
					let voters = Staking::voters(None)
						.unwrap()
						.into_iter()
						.find(|(w, _, t)| x == *w && t[0] == *w)
						.unwrap_or_default();
				...
				.collect();
			// let esg_weight = voters.into_iter().map(|(_, esg, _) | esg ).collect();
			assert_eq!(esg_weight, vec![1, 2, 3]);
		});
	}
	#[test]
